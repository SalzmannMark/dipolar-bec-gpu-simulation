...
namespace quench {
            ...
            : Ip(input), do_Imaginary_time_evolution(a), num_Imaginary_time_steps(n)  // copy the whole struct
    {
        ...
        import_parameters_with_omega_database();
    ...
    // Loading in the optimal values for omegax, omegay from the database located at "MyHeaders/trap_parameters.txt"
    inline std::vector<OmegaDatabase> GradientDescentSolver::load_trap_parameters(const std::string& filename)
    {
        std::ifstream f(filename);
        std::vector<OmegaDatabase> traps;
    ...
    inline void GradientDescentSolver::display() const {
        ...
        inline void GradientDescentSolver::import_parameters_with_omega_database()
    {   
        ...
        {
            {
                ...
                std::filesystem::path dest = parent / Ip.initial.type / ("N_" + number_value_str + "_cutoff_" + std::to_string(Ip.cutoff.use_cutoff) + "with_Itime_") / ("r_" + radius_value_str + "_edd_" + edd_initial_str);
        ...
        {
                std::filesystem::path parent = std::filesystem::path(main_directory).parent_path();
                std::filesystem::path dest = parent / Ip.initial.type / ("N_" + number_value_str + "_cutoff_" + std::to_string(Ip.cutoff.use_cutoff)) / ("r_" + radius_value_str + "_edd_" + edd_initial_str);
                destination_folder = dest.string();
        }
        ...
        auto params_from_OmegaDatabase = [this](const Trap& trap, const std::vector<OmegaDatabase>& omega_data, int number_of_particles) -> double {
            bool found = false;
            for (const auto& trap_db : omega_data){
        ...
        std::vector<OmegaDatabase> omega_data = load_trap_parameters(trap_file);


        Ip.initial.omegax = Ip.initial.omegay = params_from_OmegaDatabase(Ip.initial, omega_data, Ip.phys.number_of_particles);
        Ip.final_.omegax  = Ip.final_.omegay  = params_from_OmegaDatabase(Ip.final_, omega_data, Ip.phys.number_of_particles);
        {
            ...
            bool found_initial_scattering_length = false, found_final_scattering_length = false;
            ...
            if (infile.is_open()) {
                ...
                while (std::getline(infile, line)) {
                    ...
                    } else if(line.find("initial scattering length =") == 0) {
                        buffer << "initial scattering length = " << Ip.phys.dipolar_length/Ip.initial.edd << "\n";
                        found_initial_scattering_length = true;
                    } else if(line.find("final scattering length =") == 0) {
                        buffer << "final scattering length = " << Ip.phys.dipolar_length/Ip.final_.edd << "\n";
                        found_final_scattering_length = true;
                    } else {
                        buffer << line << "\n";
                    }
                }
            ...
            if (!found_initial_scattering_length) buffer << "initial scattering length = " << Ip.phys.dipolar_length/Ip.initial.edd << "\n";
            if (!found_final_scattering_length)   buffer << "final scattering length = " << Ip.phys.dipolar_length/Ip.final_.edd << "\n";
                        ...
                        const Vector<double>& x, const Vector<double>& y, const Vector<double>& z)
    {
        ...
        std::cout << "Initial trap steepness: " << Ip.initial.steepness << std::endl;
        std::cout << "Final trap steepness: " << Ip.final_.steepness << std::endl;
        std::cout << "----------------------------------------------------------------------" << std::endl;
    ...
    inline void GradientDescentSolver::start_quench(){
        ...
        if (Ip.cutoff.use_cutoff == 1)
        {
            ...
            std::cout << "starting the numerical gradient descent   with cutoff" << std::endl;
        ...
        if (Ip.cutoff.use_cutoff == 0)
        {
            ...
            std::cout << "starting the numerical gradient descent without cutoff" << std::endl;
            std::tie(psi, chemical_potential) = dipolar_gp_solver.run_gradient_descent(Ip.algo.num_initial_graddesc_steps,
                                                                                    //residual,
                                                                                    Ip.algo.alpha,
                                                                                    Ip.algo.beta,
